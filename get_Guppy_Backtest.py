# %run "/Users/tfoster/Documents/p/fin/Data/System/Guppy/get_Guppy_Backtest.py"
# TKF
# Created: 12.28.2016
...
def calc_sma_price(df, sma1, sma2):
	# Slow Guppy
	df.loc[:, 'SMA3'] = simple_moving_average(df['Close'], 3)
	df.loc[:, 'SMA5'] = simple_moving_average(df['Close'], 5)
	df.loc[:, 'SMA8'] = simple_moving_average(df['Close'], 8)
	df.loc[:, 'SMA10'] = simple_moving_average(df['Close'], 10)
	df.loc[:, 'SMA12'] = simple_moving_average(df['Close'], 12)
	df.loc[:, 'SMA15'] = simple_moving_average(df['Close'], 15)	
	# Fast Guppy
	df.loc[:, 'SMA30'] = simple_moving_average(df['Close'], 30)
	df.loc[:, 'SMA35'] = simple_moving_average(df['Close'], 35)
	df.loc[:, 'SMA40'] = simple_moving_average(df['Close'], 40)
	df.loc[:, 'SMA45'] = simple_moving_average(df['Close'], 45)
	df.loc[:, 'SMA50'] = simple_moving_average(df['Close'], 50)
	df.loc[:, 'SMA60'] = simple_moving_average(df['Close'], 60)		
	return 
...
def is_sma(df, starting_time_period):
	...
	df.loc[(df['SMA3'] > df['SMA5']) & (df['SMA5'] > df['SMA8'])
		& (df['SMA8'] > df['SMA10']) & (df['SMA10'] > df['SMA12'])
		& (df['SMA12'] > df['SMA15']) & (df['SMA15'] > df['SMA30'])
		& (df['SMA30'] > df['SMA35']) & (df['SMA35'] > df['SMA40'])
		& (df['SMA40'] > df['SMA45']) & (df['SMA45'] > df['SMA50'])
		& (df['SMA50'] > df['SMA60']), 'IsSma'] = 1 	# LONG
	df.loc[(df['SMA3'] < df['SMA5']) & (df['SMA5'] < df['SMA8'])
		& (df['SMA8'] < df['SMA10']) & (df['SMA10'] < df['SMA12'])
		& (df['SMA12'] < df['SMA15']) & (df['SMA15'] < df['SMA30'])
		& (df['SMA30'] < df['SMA35']) & (df['SMA35'] < df['SMA40'])
		& (df['SMA40'] < df['SMA45']) & (df['SMA45'] < df['SMA50'])
		& (df['SMA50'] < df['SMA60']), 'IsSma']	= -1 	# SHORT
	return	
...
def is_sma_reversal(df, i):
	'''Are Gupppy SMAs properly aligned?'''
...
def is_stop_violation(df, i, atr_factor):
	'''Does price exceed the stop loss?'''
...
def main(show_stats, source_data_file_path, destination_file_path, initial_equity, pct_equity_risk, sma1_period, sma2_period, atr_periods, atr_factor, start_date, end_date, ticker, output_stats_file, output_data_file_total):
	...
	for i in range(starting_time_period, len(prices)):
		if (prices.ix[i, 'OpenClose'] == 1) & (prices.ix[i-1, 'OpenClose']!=1): # Open a new position
			prices.ix[i, 'Qty'] = (prices.ix[i-1, 'Equity'] * pct_equity_risk / (prices.ix[i, 'Open'] - prices.ix[i-1, 'ExitStop'])).round(-4)#(atr_factor * prices.ix[i-1, 'ATR'])).round(-4) # QTY 
